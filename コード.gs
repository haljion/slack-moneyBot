...
+ "\n[支出種別コード]:1→食材費  2→外食費  3→日用品  4→交通費  5→医療費  6→交際費  7→趣味  8→家賃  9→光熱費  10→借金返済  11→その他";// 使用方法
  ...
  switch(datas[0]){
    case "支出":
      if(5 <= dataLength && dataLength <= 7
        ...
        sheet.getRange("B" + next_row).setValue(month + "/" + date);// 日付
          ...
          switch(datas[1]){
            ...
            case "3":
              ...
              if(dataLength == 7) {
                ...
                payoff_sheet.getRange("B" + payoff_row).setValue(datas[6]);// 日付
              } else {
                payoff_sheet.getRange("B" + payoff_row).setValue(month + "/" + date);// 日付
              }
